...
chicagoMVT <- chicagoMVT %>% 
  extract(data = ., col = Location, into = c('Latitude', 'Longitude'), 
          regex = '\\(([^,]+), ([^)]+)\\)')
chicagoMVT$Longitude <- round(as.numeric(chicagoMVT$Longitude), 5)
chicagoMVT$Latitude <- round(as.numeric(chicagoMVT$Latitude), 5)
```
...
map
map <- get_googlemap("taipei")
ggmap(map, maprange = T)
...
```{r}
library(tmaptools)
ggmap(get_stamenmap(rbind(as.numeric(paste(geocode_OSM("chicago")$bbox))), zoom = 11))
map <- get_map(location = 'Taiwan', zoom = 7)

mutate(provide_new_column = bemp - lag(bemp))
```
