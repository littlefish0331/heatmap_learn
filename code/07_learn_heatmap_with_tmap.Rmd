...
儘管可能會為不贊成使用的函數或參數給出一些消息或警告，但繪圖方法的代碼是完全向後兼容的。  
...
此代碼替換了tmap和tmaptools 2.0版的腳本v84i06.R。  
產生的數字與文章中包含的數字相同。
...
m1 <- 
  ...
  tm_style("gray") +
  tm_format(format = "World", frame.lwd = 2)
...
tmap_save(tm = m2, filename = "../data/classic.png",
          width = 6.125, 
          scale = .7, dpi = 300, outer.margins = 0)
```
